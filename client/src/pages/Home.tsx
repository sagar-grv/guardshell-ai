      ...
      <header className="relative z-10 border-b border-white/8 bg-[#0A0F13]/86 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-[1600px] items-center justify-between px-5 md:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-baseline gap-1 font-display text-lg font-bold tracking-[-0.06em] text-white"><span>Guard</span><span className="text-[#72F2B2]">/</span><span>Shell</span><span className="ml-2 font-mono text-[9px] font-semibold tracking-[0.22em] text-[#72F2B2]">AI</span></div>
              <p className="font-mono text-[10px] tracking-[0.13em] text-[#88A0A5]">PROMPT · POLICY · PROOF</p>
            </div>
          </div>
        ...
        <section className="grid gap-7 xl:grid-cols-[230px_minmax(0,1fr)_310px]">
          ...
          <div className="order-1 min-w-0 xl:order-2">
            <section className="relative overflow-hidden rounded-[28px] border border-white/10 bg-[#0E171B] shadow-[0_30px_90px_rgba(0,0,0,0.25)]">
              <div className="absolute inset-0 opacity-35" style={{ backgroundImage: "linear-gradient(rgba(114,242,178,0.035) 1px, transparent 1px), linear-gradient(90deg, rgba(114,242,178,0.035) 1px, transparent 1px)", backgroundSize: "40px 40px" }} />
              <div className="absolute right-8 top-8 h-24 w-24 border border-[#72F2B2]/15" />
              <div className="absolute right-14 top-14 h-12 w-12 border border-[#72F2B2]/30" />
              <div className="relative p-5 sm:p-7 lg:p-9">
                <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
                  <div className="max-w-xl"><div className="mb-3 flex items-center gap-2"><span className="h-px w-8 bg-[#72F2B2]" /><span className="font-mono text-[10px] font-semibold tracking-[0.22em] text-[#72F2B2]">ACTIVE COMMAND WORKBENCH</span></div><h1 className="font-display max-w-2xl text-2xl font-bold leading-tight tracking-[-0.035em] text-white sm:text-3xl">Command inspection, before execution.</h1><p className="mt-2 max-w-xl text-sm leading-6 text-[#AEC0C4]">Inspect syntax, context, policy signals, and the safest next move on one calibrated rail.</p></div>
                  <div className="hidden border border-[#72F2B2]/20 bg-[#72F2B2]/5 px-3 py-2 sm:block"><p className="font-mono text-[9px] tracking-[0.18em] text-[#72F2B2]">EVD-CORE/01</p><p className="mt-1 font-mono text-[10px] text-[#88A0A5]">CHAIN OF CUSTODY ACTIVE</p></div>
                </div>
                <div className="relative border border-[#72F2B2]/45 bg-[#07110F]/95 p-2 shadow-[inset_0_0_0_1px_rgba(114,242,178,0.06),0_0_24px_rgba(114,242,178,0.07)]">
                  <div className="absolute bottom-0 left-0 top-0 w-1 bg-[#72F2B2]" />
                  <div className="flex items-center gap-2 px-3 pb-1.5 pt-1"><TerminalSquare size={15} className="text-[#72F2B2]" /><span className="font-mono text-[10px] tracking-[0.18em] text-[#73A994]">COMMAND BEAM // INGRESS → INSPECT → DECIDE</span><span className="ml-auto font-mono text-[10px] text-[#547167]">POLICY: STRICT</span></div>
                  <div className="flex flex-col gap-2 md:flex-row"><div className="flex min-w-0 flex-1 items-center gap-3 border border-white/7 bg-black/40 px-4 py-3"><span className="font-mono text-sm text-[#72F2B2]">$</span><input aria-label="Linux command" value={command} onChange={(event) => setCommand(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") runAnalysis(); }} className="min-w-0 flex-1 bg-transparent font-mono text-sm text-white outline-none placeholder:text-[#59716D]" placeholder="e.g. rm -rf project/*" /></div><button onClick={runAnalysis} className="inline-flex items-center justify-center gap-2 border border-[#B7FFD7] bg-[#72F2B2] px-5 py-3 font-display text-sm font-bold text-[#07110C] transition duration-200 hover:bg-[#9CF6C6] active:scale-[0.97]"><Sparkles size={16} />Inspect command</button></div>
                </div>
            ...
            <section className={`mt-5 overflow-hidden rounded-[26px] border bg-[#10181C]/92 shadow-[0_18px_60px_rgba(0,0,0,0.18)] ${riskBorder(analysis.risk)}`}>
              ...
              <div className="p-5 sm:p-6">
                ...
                <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
                  ...
                  <div className="border border-white/8 bg-[#0A0F13]/72 p-4"><div className="mb-3 flex items-center gap-2"><LockKeyhole size={14} className="text-[#7AC6EF]" /><p className="font-mono text-[10px] tracking-[0.16em] text-[#8EA6AB]">POLICY EVIDENCE</p><span className="ml-auto font-mono text-[9px] text-[#5E767B]">EVD-{analysis.score.toString().padStart(3, "0")}</span></div><div className="space-y-2">{analysis.evidence.map((evidence) => <div key={evidence} className="flex items-start gap-2 text-xs leading-5 text-[#B4C7CA]"><Check size={14} className="mt-0.5 shrink-0 text-[#72F2B2]" />{evidence}</div>)}</div></div>
                </div>
          ...
          <aside className="order-3 flex flex-col gap-4">
            <div className="rounded-[22px] border border-white/8 bg-[#10181C]/82 p-4 shadow-[0_22px_65px_rgba(0,0,0,0.2)] backdrop-blur-sm"><div className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2"><History size={15} className="text-[#7AC6EF]" /><span className="font-mono text-[10px] tracking-[0.18em] text-[#7E9499]">LOCAL AUDIT</span><span className="font-mono text-[9px] text-[#4E686D]">CUSTODY LOG</span></div>{audit.length > 0 && <button onClick={clearAudit} className="font-mono text-[10px] text-[#82979B] transition hover:text-[#FF959B]">CLEAR</button>}</div>{audit.length === 0 ? <div className="rounded-xl border border-dashed border-white/10 px-3 py-8 text-center"><Clock3 size={18} className="mx-auto mb-2 text-[#668085]" /><p className="font-mono text-[9px] tracking-[0.12em] text-[#667D82]">EVD-AWAITING-INPUT</p><p className="mt-2 text-xs leading-5 text-[#82979B]">Your analyzed commands will appear here.</p></div> : <div className="max-h-[365px] space-y-2 overflow-auto pr-1">{audit.map((entry, index) => <div key={`${entry.timestamp}-${index}`} className="rounded-xl border border-white/7 bg-black/20 p-3"><div className="mb-2 flex items-center justify-between"><div className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: riskMeta[entry.risk].color }} /><span className="font-mono text-[9px] tracking-[0.14em]" style={{ color: riskMeta[entry.risk].color }}>{riskMeta[entry.risk].short}</span><span className="font-mono text-[9px] text-[#4E686D]">EVD-{String(audit.length - index).padStart(3, "0")}</span></div><span className="font-mono text-[9px] text-[#71878C]">{formatTime(entry.timestamp)}</span></div><p className="truncate font-mono text-xs text-[#D6E6E7]">$ {entry.command}</p></div>)}</div>}</div>
